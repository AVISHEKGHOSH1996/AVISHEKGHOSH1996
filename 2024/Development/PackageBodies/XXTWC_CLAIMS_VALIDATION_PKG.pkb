...
function validation_for_rebill_price(
 p_claim_id in number
	...
	for i in(select
                rownum,
                unit_rebill_price as rebill_price,
                claim_qty         as claim_qty
            from
                xxtwc_claims_lines
            where
                claim_id = p_claim_id)
				loop
				if (i.rebill_price is null or i.rebill_price < 0) and i.claim_qty > 0 then 
					lv_line := lv_line || i.rownum || ',';
                ...
                lv_error:= 'Rebill Price cannot be Null or Negative. Check Line :- '||rtrim(lv_line,',');
                end if;
	
	return lv_error;
	end validation_for_rebill_price;
    -------------------------------------------------------------
	...
	from
		...
		where 
		     header_id = i.attribute1
        and fulfill_line_id = i.fulfillment_line_id
        and claim_id = p_claim_id;
	
	if lv_cnt <> i.claim_qty then 
	
	lv_line := lv_line || i.claim_line_no || ',';
	end if;
	
	end loop;
	
	if lv_line is not null then
                lv_error:= 'Claim qty total is not matching in line and lot level. Ref line no. :' ||lv_line;		 
	end if;	
    return lv_error;
	end validation_for_claim_qty_sum;
...
function validation_for_claim_qty_negative(
 p_claim_id in number
	...
	for i in(select
                 ...
                 claim_qty  from XXTWC_CLAIMS_LPN_LINES
        where 
        CLAIM_ID = p_claim_id 
            )
				loop
				if (i.claim_qty < 0) then 
					lv_line := 1;
                ...
                if lv_line is not null then
                lv_error:= 'Claim qty should not be negative in lot level';
                end if;
	
	return lv_error;
	end validation_for_claim_qty_negative;
