...
--        raise_application_error('-20000','p_claim_type-'||p_claim_type);
        ...
        /*Fix No Data Found LPN Issue */
            BEGIN
             SELECT COUNT(1) , sum(requested_quantity)
               INTO lv_count, lv_line_sum
          ...
          IF lv_count >0 THEN
            IF lv_line_sum = e_rec.shipped_qty
            THEN
              FOR e_rec_lpn1 IN (
                 SELECT
                    *
            ...
            ELSE
              FOR e_rec_lpn1 IN (
                 SELECT
                    *
                  FROM
                     XXTWC_CLAIMS_ORD_OT_LPN_LINES_V  im
                    WHERE
                    im.header_id = p_header_id
                    and im.fulfill_line_id = e_rec.fulfill_line_id
                    )
...
WHERE
	...
	(CH.WF_NEXT_SEQ_NO = (SELECT WD.SEQ FROM XXTWC_CLAIMS_APPR_WF_DETAILS WD 
                                     ...
                                     AND UPPER(WD.STEP_NAME) = UPPER('Missing Lot Approval'))
      AND CH.MISSING_LOT = 'Y'                                
	AND (( EXISTS(SELECT 1
	...
	AND G.GROUP_NAME = 'Import Claim Owner Approval'
    AND GU.STATUS = 1
    AND LG.ACTIVE_FLG = 'Y'
	AND LOWER(LG.EMAIL_ID) = LOWER(p_user_name))
          ...
          AND UR.USER_ID = V('P0_USER_ID')
          AND CR.ROLE_NAME = 'Super Approver')
		 
				)	OR EXISTS(SELECT 1
          FROM CLAIMS.XXTWC_CLAIMS_REASSIGN_APPROVAL RA
          WHERE  RA.CLAIM_ID = CH.CLAIM_ID
	      AND RA.WF_NEXT_SEQ_NO = CH.WF_NEXT_SEQ_NO
  ...
  (CH.WF_NEXT_SEQ_NO = (SELECT WD.SEQ FROM XXTWC_CLAIMS_APPR_WF_DETAILS WD 
    ...
    WHERE UPPER(EMAIL_ID) = UPPER(ch.salesmanager) AND ACTIVE_FLG = 'Y') AND LOWER(p_user_name) = LOWER(ch.salesmanager))
	OR (exists(SELECT 
	USER_ID
	FROM XXTWC_CLAIMS_USER_LOGIN_DETAILS
	...
	AND ACTIVE_FLG = 'Y')AND LOWER(p_user_name) = (SELECT 
	LOWER(EMAIL_ADDRESS) 
    FROM fusion.jtf_rs_salesreps jrs,
        fusion.hz_parties hp_sales
    WHERE jrs.resource_id = hp_sales.party_id  
    ...
    FROM XXTWC_CLAIMS_USER_LOGIN_DETAILS
    WHERE UPPER(EMAIL_ID) = UPPER(ch.salesmanager)AND ACTIVE_FLG = 'Y' /*AND LOWER(:P0_USER_NAME) = LOWER(ch.salesmanager)*/) 
    ...
    AND SYSDATE BETWEEN jrs.start_date_active and jrs.end_date_active
    and hp_sales.party_id = CH.SALES_PERSON_ID)
	AND ACTIVE_FLG = 'Y' /*AND LOWER(:P0_USER_NAME) = (SELECT 
	LOWER(EMAIL_ADDRESS) 
    FROM fusion.jtf_rs_salesreps jrs,
        fusion.hz_parties hp_sales
    WHERE jrs.resource_id = hp_sales.party_id  
	...
	OR exists(SELECT 1
          ...
          OR 	(CH.WF_NEXT_SEQ_NO = (SELECT WD.SEQ FROM XXTWC_CLAIMS_APPR_WF_DETAILS WD 
                                     WHERE WD.WF_ID = CH.WF_ID 
                                     AND UPPER(WD.STEP_NAME) = UPPER('Finance Approval'))
	AND ((LOWER(p_user_name) IN (SELECT LOWER(EMAIL_ID)
	...
	(SELECT GROUP_ID
          ...
          AND CR.ROLE_NAME = 'Super Approver')
          /*OR EXISTS(SELECT 1
          FROM CLAIMS.XXTWC_CLAIMS_REASSIGN_APPROVAL RA, CLAIMS.XXTWC_CLAIMS_USER_LOGIN_DETAILS LD
          WHERE RA.REASSIGN_TO = LD.USER_ID
          AND RA.CLAIM_ID = CH.CLAIM_ID
	      AND RA.WF_NEXT_SEQ_NO = CH.WF_NEXT_SEQ_NO
	      AND RA.REASSIGN_TO = LD.USER_ID 
           AND RA.STATUS = 1
            )*/
				)	OR EXISTS(SELECT 1
          FROM CLAIMS.XXTWC_CLAIMS_REASSIGN_APPROVAL RA
