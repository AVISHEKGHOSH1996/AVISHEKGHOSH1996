...
--        raise_application_error('-20000','p_claim_type-'||p_claim_type);
        ...
        FOR e_rec IN (
            SELECT
                *
            FROM
                  XXTWC_CLAIMS_ORDER_LPN_LINES_MV l --xxtwc_claims_order_lpn_lines_v l
                WHERE
                l.header_id = p_header_id
                and l.fulfill_line_id in (select fulfill_line_id from xxtwc_claims_order_lines_v  where header_id = p_header_id)
        ) LOOP
            apex_collection.add_member(
                p_collection_name => 'XXTWC_CLAIMS_LOTS', 
                p_c001 => e_rec.source_header_number, 
                p_c002 => e_rec.order_number, 
                p_c003 => e_rec.header_id,
                p_c004 => e_rec.fulfill_line_id,
                p_c005 => e_rec.delivery_name,
                p_c006 => e_rec.lot_number, 
                p_c007 => e_rec.lpn_number, 
                p_c008 => e_rec.requested_quantity, 
                p_c009 => 0,
                p_c010 =>e_rec.ORIGINAL_LOT_NUMBER,
                p_c011 =>e_rec.ORIGINAL_LPN_NUMBER,
                p_c012 =>e_rec.RANCH_BLOCK,
                p_c013 =>e_rec.POOL_NUMBER,
                p_c014 =>e_rec.PACK_DATE
            );
        END LOOP;
    exception
        when others then 
